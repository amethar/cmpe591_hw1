...
def collect(train_dataset_size, test_dataset_size):
    ...
    #positions = torch.zeros(train_dataset_size, 2, dtype=torch.float) #final positions are not used
    ...
    #torch.save(positions, "p2_train_dataset_reference_outputs.pt") #final positions are not used
    torch.save(actions, "p3_train_dataset_actions.pt")
    torch.save(imgs_before, "p3_train_dataset_imgs_before.pt")
    torch.save(imgs_after, "p3_train_dataset_imgs_after.pt") 



    #collect test data
    #positions = torch.zeros(test_dataset_size, 2, dtype=torch.float) #final positions are not used
    ...
    #torch.save(positions, "p2_test_dataset_reference_outputs.pt") #final positions are not used
    torch.save(actions, "p3_test_dataset_actions.pt")
    torch.save(imgs_before, "p3_test_dataset_imgs_before.pt")
    torch.save(imgs_after, "p3_test_dataset_imgs_after.pt") 
...
class CNNModel(nn.Module):
    def __init__(self):
        ...
        self.conv_layers = nn.Sequential(
            ...
            nn.Conv2d(3, 8, kernel_size=3, stride=1, padding=1),  #resulting size 128x128x8
            nn.ReLU(),
            ...
            nn.Conv2d(16, 32, kernel_size=3, stride=1, padding=1), #resulting size 32x32x32
            nn.ReLU(),
        ...
        # getting the resulting image instead of positions
        self.deconv_layers= nn.Sequential(

            nn.ConvTranspose2d(4, 16, kernel_size=4, stride=4, padding=0, output_padding=0),  # resulting size 16x16x16
            nn.ReLU(),

            nn.ConvTranspose2d(16, 3, kernel_size=8, stride=8, padding=0, output_padding=0),  # resulting size 128x128x3
    ...
    def forward(self, img, action):
        ...
        # deconvolution to get output image
...
def train():
    ...
    images = torch.load("p3_train_dataset_imgs_before.pt").float() #convolutional layer requires float tensors
    images = images / 255.0 # I constantly get normalization suggestions, but not sure about the reason
    actions = torch.load("p3_train_dataset_actions.pt")
    reference_outputs = torch.load("p3_train_dataset_imgs_after.pt").float() / 255.0
    ...
    #save the model
    torch.save(model, "hw1_3.pt")


def test():
    model = torch.load("hw1_3.pt", weights_only=False) #load the model
    model.eval()  # set to evaluation mode

    # load the test dataset
    images = torch.load("p3_test_dataset_imgs_before.pt").float() #convolutional layer requires float tensors
    images = images / 255.0 # I constantly get normalization suggestions, but not sure about the reason
    actions = torch.load("p3_test_dataset_actions.pt")
    reference_outputs = torch.load("p3_test_dataset_imgs_after.pt").float() / 255.0
    ...
    num_examples = 5  # number of examples images to show
    fig, axes = plt.subplots(num_examples, 2, figsize=(9, 2 * num_examples))

    for i in range(num_examples):
        idx = np.random.randint(0, num_samples)

        # converting to NumPy format
        ref_img = reference_outputs[idx].permute(1, 2, 0).cpu().numpy()
        pred_img = predictions[idx].permute(1, 2, 0).cpu().numpy()
